...
# --- [BARU] Fungsi Header Global ---
def show_app_header():
    """Menampilkan header aplikasi yang konsisten di semua halaman setelah login."""
    st.markdown("""
    <style>
        .header-title { font-size: 2.2rem; font-weight: bold; color: #1e1e1e; margin-bottom: 0px; }
        .header-welcome { font-size: 1.1rem; color: #444; margin-top: 0px; }
    ...
    with col3:
        ...
        with st.container():
            ...
            if st.button("Logout", key="btn_logout_global", use_container_width=True):
                # Hapus semua state sesi untuk logout bersih
...
# --- [DIMODIFIKASI] Dasbor Utama (tanpa header) ---
def show_main_dashboard():
    ...
    conn = db.create_connection()
    try:
        total_siswa = db.get_total_siswa_aktif(conn)
        total_tunggakan = db.get_total_tunggakan(conn)
        pemasukan_hari_ini = db.get_pemasukan_hari_ini(conn)
...
# --- [DIMODIFIKASI] Router Utama ---
def main():
    if not st.session_state.logged_in:
        show_login_page()
    else:
        # Panggil header global di sini, akan muncul di semua halaman
        show_app_header()

        # Lanjutkan dengan logika navigasi halaman
        ...
        if page == 'home':
            ...
            if module_to_render and hasattr(module_to_render, 'render'):
                # Anda bisa menambahkan judul modul jika perlu
                st.markdown("---")
                st.header(f"Modul: {page.replace('_', ' ').title()}")
